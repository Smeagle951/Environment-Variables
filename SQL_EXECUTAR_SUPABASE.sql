-- =====================================================
-- VIEWS PARA APP ADMIN DASHBOARD
...
SELECT 
  ...
  CASE 
    WHEN EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'farms')
    THEN (SELECT COUNT(*) FROM public.farms WHERE group_id = fg.id AND is_active = true)
    ELSE 0
  END as farm_count,
  ...
  CASE 
    ...
    THEN GREATEST(
      fg.updated_at,
      MAX(gm.updated_at),
      COALESCE((SELECT MAX(updated_at) FROM public.farms WHERE group_id = fg.id), fg.updated_at)
    )
    ELSE GREATEST(
      fg.updated_at,
      MAX(gm.updated_at)
...
COMMENT ON VIEW admin_groups_summary IS 'Resumo completo de todos os grupos para dashboard admin';
